...
It talks to the pipeline API (default http://localhost:8000), pauses at both
approval gates to ask you what to do, and saves the finished video to
...
def keyword_gate(api: Api, job: dict) -> dict:
    while True:
        ...
        if ans == "no":
            ...
            job = api.call("POST", f"/jobs/{job['id']}/keywords/reject", {"feedback": note})
        ...
        return api.call("POST", f"/jobs/{job['id']}/keywords/review",
                        {"approved_ids": chosen, "extra_terms": [t for t in extra.split(",") if t.strip()]})


def scene_gate(api: Api, job: dict) -> dict:
    while True:
        print("\n=== GATE 2: scenes ===  (why: this is your last chance before the slow render)\n")
        print("SCRIPT:\n" + job["script"] + "\n")
        for i, s in enumerate(job["scenes"], 1):
            clip = Path(s["clip_path"]).name if s.get("clip_path") else "(NO CLIP - add footage to library/clips)"
            print(f"  {i}. [{clip}]\n     {s['narration']}")
        ...
        if order:
            try:
                ...
                if sorted(idx) != list(range(1, n + 1)):
                    raise ValueError
                job = api.call("PATCH", f"/jobs/{job['id']}/scenes", {"order": [job["scenes"][i - 1]["id"] for i in idx]})
        ...
        if ans in ("y", "yes"):
            return api.call("POST", f"/jobs/{job['id']}/scenes/approve")
        note = ask("What should change? (guides the rewrite): ")
        job = api.call("POST", f"/jobs/{job['id']}/scenes/reject", {"feedback": note})
...
def main() -> None:
    ...
    job = api.call("POST", "/jobs", {"subject": subject, "providers": {"keywords": args.keywords}})
    print(f"Created job {job['id']}")
    api.call("POST", f"/jobs/{job['id']}/start")

    job = wait_for(api, job["id"], {"keywords_review"}, "researching keywords")
    job = keyword_gate(api, job)
    job = wait_for(api, job["id"], {"scenes_review"}, "writing script and picking clips", every=4)
    job = scene_gate(api, job)
