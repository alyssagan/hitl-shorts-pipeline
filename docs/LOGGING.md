...
`"reused_from_file"` in `logic`) both show up here the same way an LLM's proposal does. See docs/RUNNING.md
"Keyword files, always" for the full shape and where the reusable per-subject library lives.
